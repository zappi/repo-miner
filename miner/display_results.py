...
def classify_churn(churn_data, relative_threshold=0.2):
    ...
    for file_path, file_data in churn_data["file_churn"].items():
        ...
        if avg_relative_churn < relative_threshold:
            good_churn.append(file_path)
        else:
            bad_churn.append(file_path)

    return good_churn, bad_churn
...
def report_code_churn(churn_data, developer_contribution):
    ...
    # Classify churn as good or bad
    good_churn, bad_churn = classify_churn(churn_data)

    print(f"\nGood Churn Files: {len(good_churn)}")

    print(f"\nBad Churn Files: {len(bad_churn)}")
